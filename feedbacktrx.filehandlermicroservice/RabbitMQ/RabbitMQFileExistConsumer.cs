...
public class RabbitMQFileExistConsumer : BackgroundService
{
    ...
    public RabbitMQFileExistConsumer(string rabbitMQConnectionString, string exchangeName, string queueName, string routingKey, string username, string password, IServiceProvider serviceProvider)
    {
    ...
    private async Task<string> ProcessMessage(string message)
    {
        using (var scope = _serviceProvider.CreateScope())
        {
            ...
            return await CheckIfFileExists(fileHandlerService, message);
        }
    }

    private async Task<string> CheckIfFileExists(IFileHandlerService fileHandlerService, string message)
    {
        bool result = await fileHandlerService.CheckIfFileExists(message);
